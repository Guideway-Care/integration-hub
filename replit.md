...
- **Operator Pause holds back the scheduled daily download (product decision, July 2026)**: While a paused marker exists (`isDownloadPaused()` in `routes/bq.ts`), the 6:30 AM contacts daily run still extracts + transforms, but skips the queue/download phases and records the run as `skipped` (shown as "skipped (paused)" on the dashboard/InContact pages); `/bq/run-job` also 409s while paused. Resume (`/bq/run-job-resume` or ad-hoc resume) ends the paused state; if the queue is empty, `run-job-resume` clears the daily paused marker so run-job can't be deadlocked. Contacts skipped this way are picked up by the next unpaused daily run via `findPendingRecordingContactIds`.
